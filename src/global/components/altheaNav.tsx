...
export const AltheaNav = () => {
  const networkInfo = useNetworkInfo();
  const alert = useAlert();
  const { activateBrowserWallet, account, chainId, active } = useEthers();
  const balance = useEtherBalance(account);
  const altheaBalance = useEtherBalance(account, {
    chainId: getAltheaNetwork(Number(networkInfo.chainId)).chainId,
  });
  ...
  useEffect(() => {
    ShowAlerts(
      ...
      canPubKey,
      account,
      altheaBalance ?? BigNumber.from(0),
      location.pathname,
      pageList
    );
  }, [
