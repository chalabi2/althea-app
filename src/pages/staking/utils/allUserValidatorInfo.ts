...
export async function getStakingApr(nodeAddressIP: string) {
  const urlInflation = "https://nodes.chandrastation.com/testnet/api/althea/" + "/cosmos/mint/v1beta1/annual_provisions";
  const urlStake = "https://nodes.chandrastation.com/testnet/api/althea/" + "/cosmos/staking/v1beta1/pool";
...
export async function getSafeVals(nodeAddressIP: string) {
  const urlValidators = "https://nodes.chandrastation.com/testnet/api/althea//cosmos/staking/v1beta1/validators";
...
async function getValconsAddressForValidator(
  ...
  try {
    ...
    const url = `https://nodes.chandrastation.com/testnet/api/althea//cosmos/staking/v1beta1/validators/${operatorAddress}`;
...
export async function getValconsAddresses(nodeAddressIP: string): Promise<string[]> {
  try {
      const valoperAddresses = (await getSafeVals("https://nodes.chandrastation.com/testnet/api/althea/")).notJailed;
      ...
      const consensusKeysPromises = valoperAddresses.map(async (valoperInfo) => {
          const url = `https://nodes.chandrastation.com/testnet/api/althea//cosmos/staking/v1beta1/validators/${valoperInfo.operator_address}`;
...
export async function getSigningInfo(nodeAddressIP: string) {
  try {
    const valconsAddresses = await getValconsAddresses("https://nodes.chandrastation.com/testnet/api/althea/");
    ...
    const allSigningInfoPromises = valconsAddresses.map(async (address) => {
      try {
        const urlSigningInfo = `https://nodes.chandrastation.com/testnet/api/althea//cosmos/slashing/v1beta1/signing_infos/${address}`;
...
export async function getSlashingInfo(nodeAddressIP: string) {
  const notJailedValidators = (await getSafeVals("https://nodes.chandrastation.com/testnet/api/althea/")).notJailed;
  ...
  const slashingInfoPromises = notJailedValidators.map(async (valoperInfo) => {
    const urlSlashingInfo = `https://nodes.chandrastation.com/testnet/api/althea//cosmos/distribution/v1beta1/validators/${valoperInfo.operator_address}/slashes`;
...
export async function getValidatorsInfo(nodeAddressIP: string) {
  try {
    const { notJailed: notJailedValidators } = await getSafeVals("https://nodes.chandrastation.com/testnet/api/althea/");
    ...
    await Promise.all(
      notJailedValidators.map(async (valoperInfo) => {
        const valconsAddress = await getValconsAddressForValidator(valoperInfo.operator_address, "https://nodes.chandrastation.com/testnet/api/althea/");
    ...
    const [signingInfos, slashingsInfoResponse] = await Promise.all([
      getSigningInfo("https://nodes.chandrastation.com/testnet/api/althea/"),
      getSlashingInfo("https://nodes.chandrastation.com/testnet/api/althea/")
    ]);
